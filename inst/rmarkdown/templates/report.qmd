...
power_without_fut <- power(x = params$sim_power, adjust_for_futility = FALSE, level = conf.level)
power_with_fut <- power(x = params$sim_power, adjust_for_futility = TRUE, level = conf.level)

  fut.trig <- mc_error_proportion(x = sum(params$sim_power$simresults$fut.trig),
                                  n = nrow(params$sim_power$simresults),
                                  level = conf.level)

  inc.ss <- mc_error_proportion(x = sum(params$sim_power$simresults$inc.ss),
                                n = nrow(params$sim_power$simresults),
                                level = conf.level)
...
```{r}
#| echo: false
plot(params$sim$sim_power)
```

```{r}
#| echo: false
plotInterimPPoS(params$sim)
```
